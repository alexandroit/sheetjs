...
Deno.test('write features', async function(t) {
	...
	await t.step('HTML', async function(t) {
		...
		await t.step('should sanitize unsafe links only when requested', async function(t) {
			[
				...
				assert.assert(X.utils.sheet_to_html(sheet).indexOf('<a href=') > -1);
				assert.equal(X.utils.sheet_to_html(sheet, {sanitizeLinks:true}).indexOf('<a href='), -1);
			});
		});
		await t.step('should preserve safe links when sanitization is requested', async function(t) {
			["https://example.com", "http://example.com", "mailto:test@example.com", "tel:+15551234567", "/relative/path", "../report.html"].forEach(function(target) {
				var sheet = X.utils.aoa_to_sheet([["Link"]]);
				get_cell(sheet, "A1").l = {Target: target};
				assert.assert(X.utils.sheet_to_html(sheet, {sanitizeLinks:true}).indexOf('<a href=') > -1);
			});
		});
	});
...
for(var mfti = 0; mfti < mft.length; ++mfti) { let x = mft[mfti], mftcsv = csv, mftformulae = formulae;
	if(x.charAt(0)!="#") { await t.step('MFT ' + x, async function(t) {
		...
		if(mftcsv) await t.step('should have the same CSV', async function(t) {
			cmparr(f.map(function(x) { return x.SheetNames; }));
			f[0].SheetNames.forEach(function(name) {
				cmparr(f.map(function(x) { return X.utils.sheet_to_csv(x.Sheets[name]); }));
			});
		});
		if(mftformulae) await t.step('should have the same formulae', async function(t) {
			cmparr(f.map(function(x) { return x.SheetNames; }));
